...
output "subnet_ids" {
  ...
  value       = flatten(concat([for s in azurerm_subnet.snet : s.id], [var.gateway_subnet_address_prefix != null ? azurerm_subnet.gw_snet.*.id : null], [var.firewall_subnet_address_prefix != null ? azurerm_subnet.fw-snet.*.id : null]))
}
...
output "dns_zone_id" {
  value       = element(concat(azurerm_dns_zone.dns-zone.*.id, [""]), 0)
  description = "The DNS Zone created alongside the VNET."
}
